...
io.on("connection", (socket) => { // When a user coonnects to our server
    socket.on("join-room", (roomId, id, myname) => { // When the socket a event 'join room' event
        ...
        socket.broadcast.to(roomId).emit("user-connected", id, myname);// emit a 'user-connected' event to tell all the other users
        ...
        socket.on("captions", (captionData) => {
            console.log("자막자막자막!!!!!!", captionData);
            socket.broadcast.to(roomId).emit("receive-captions", captionData);
          });
...
// // process.env.PORT || 3030 means  use port 3000 unless there exists a preconfigured por
